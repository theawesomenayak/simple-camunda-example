...
public abstract class TaskHandler implements ExternalTaskHandler {
  ...
  public final void execute(final ExternalTask externalTask,
    ...
    try {
      ...
      log.info(LOG_FORMAT, externalTask.getProcessInstanceId(), externalTask.getId(), Status.COMPLETED);
    } catch (final Exception e) {
      externalTaskService.handleBpmnError(externalTask, e.getMessage());
      log.error(LOG_FORMAT, externalTask.getProcessInstanceId(), externalTask.getId(), Status.ERROR, e);
    }
  }

  protected abstract void handle(final ExternalTask externalTask, final ExternalTaskService externalTaskService);
}
